		...
		for dy in -glow_range..=glow_range {
			...
			for (px, py, &p) in text_stamp.enumerate_pixels() {
				...
				if dest_x >= 0 && dest_y >= 0 &&
					...
					let Rgba([r, g, b, a]) = p;
					let white = 255u8;
					let new_r = ((r as f32 * (1.0 - alpha_factor))
						+ (white as f32 * alpha_factor))
						.min(255.0) as u8;
					let new_g = ((g as f32 * (1.0 - alpha_factor))
						+ (white as f32 * alpha_factor))
						.min(255.0) as u8;
					let new_b = ((b as f32 * (1.0 - alpha_factor))
						+ (white as f32 * alpha_factor))
						.min(255.0) as u8;
...
fn render_cell(cell_index: u32, text_stamp: &RgbaImage, stamp_w: u32, stamp_h: u32) -> RgbaImage {
	...
	draw_cell(&mut cell_canvas, 0, 0, text_stamp, cell_index, stamp_w, stamp_h);
	cell_canvas
}

/// 並列処理で各セルをレンダリングし、セル画像を合成して横一列の画像を生成する
fn benchmark_render(letter_count: u32, stamp_w: u32, stamp_h: u32, text_stamp: &RgbaImage) -> (Duration, RgbaImage) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let threshold_secs: f64 = if args.len() > 1 { args[1].parse().unwrap_or(2.0) } else { 2.0 };
	...
	let (_elapsed, final_bench_canvas) = benchmark_render(letter_count, stamp_width, stamp_height, &text_stamp);
